...
const fetchNews = async () => {
  const fetchNews = await fetch(
    "https://newsapi.org/v2/top-headlines?country=in&from=2020-05-14&sortBy=popularity&apiKey=6f1082065f47444fb604f6801b3b7ffa&category=technology"
  );
...
function createSingleNews(articles) {
  ...
  articles.forEach((article) => {
    if (article.description != null) {
      ...
      singlenews.innerHTML = `<div class="news-thumbnail">
                                        <img src="${article.urlToImage}" >
                                     </div>
                                    <div class="news-details">
                                        <h3><a target="_blank" href='${article.url}'>${article.title}</a></h3>
                                        <div class='article-details'><p>Source: ${article.source.name}</p><p>Author: ${authortext}</p></div>
